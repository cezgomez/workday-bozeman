...
const LIST_REPORT = 'CR_Export_Benefit_Event_-_Copy';

const DETAIL_REPORT = 'CR_Export_Benefit_Event';

/**
 * benefit-event — Change Benefits event attachments (RaaS list + detail Base64)
 *
 * Requirements lines 719–798:
 *   1. CR_Export_Benefit_Event_-_Copy → EmployeeID + workdayID + WDWID
 *   2. CR_Export_Benefit_Event?Worker!WID=… → Attachment + Base64
 *   3. Match WDWID / Attachment WID
 *   4. Benefit_Event/100_{EmployeeId}_BenefitEvent_{AttachmentNoSpaces}
 *   5. SFTP /ROI/Workday/
 *
 * Example:
 *   Benefit_Event/100_40400_BenefitEvent_Divorce.pdf
 */
export async function runBenefitEvent(options = {}) {
  const {
    mock = false,
    pageSize = 5,
    maxPages = null,
  ...
  const apiName = 'benefit-event';
  const directoryName = 'Benefit_Event';
  const filePrefix = 'BenefitEvent';
  ...
  const listUrl = process.env.WORKDAY_BENEFIT_EVENT_LIST_URL || buildRaasUrl(LIST_REPORT);
  const detailUrl =
    process.env.WORKDAY_BENEFIT_EVENT_DETAIL_URL || buildRaasUrl(DETAIL_REPORT);

  try {
    if (mock) {
      throw new Error(
        'benefit-event --mock true is not implemented. Use --mock false.'
      );
    }

    const wd = getWorkdayConfig();

    console.log(`[${apiName}] step 1/2: load Benefit Event list (all employees)`);
      ...
      for (let p = 0; p < pageLimit; p++) {
        ...
        const batchResults = await mapPool(page, concurrency, async (emp) => {
          ...
          try {
            ...
            const byWid = parseBenefitEventDetailXml(detailBody);
            // Also index all base64 entries if WDWWID match fails
            ...
            // Fallback: save all detail attachments for this worker
