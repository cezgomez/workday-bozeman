...
import {
  parseBenefitEventListXml as parseDomesticPartnerListXml,
  parseBenefitEventDetailXml as parseDomesticPartnerDetailXml,
...
const LIST_REPORT = 'CR_Export_Domestic_Partnership_-_Copy';

const DETAIL_REPORT = 'CR_Export_Domestic_Partnership';

/**
 * domestic-partner — Domestic Partnership Affidavit (Life Event) attachments
 *
 * Requirements lines 879–961:
 *   1. CR_Export_Domestic_Partnership_-_Copy → EmployeeID + workdayID + WDWID
 *   2. CR_Export_Domestic_Partnership?Worker!WID=… → Attachment + Base64
 *   3. Match WDWID / Attachment WID
 *   4. Domestic_Partnership/100_{EmployeeId}_DomesticPartnership_{AttachmentNoSpaces}
 *   5. SFTP /ROI/Workday/
 *
 * Example:
 *   Domestic_Partnership/100_46250_DomesticPartnership_Scan03-29-2022-104551.pdf
 */
export async function runDomesticPartner(options = {}) {
  const {
    mock = false,
    pageSize = 5,
    maxPages = null,
  ...
  const apiName = 'domestic-partner';
  const directoryName = 'Domestic_Partnership';
  const filePrefix = 'DomesticPartnership';
  ...
  const listUrl =
    process.env.WORKDAY_DOMESTIC_PARTNER_LIST_URL || buildRaasUrl(LIST_REPORT);
  const detailUrl =
    process.env.WORKDAY_DOMESTIC_PARTNER_DETAIL_URL || buildRaasUrl(DETAIL_REPORT);

  try {
    if (mock) {
      throw new Error(
        'domestic-partner --mock true is not implemented. Use --mock false.'
      );
    }
    ...
    console.log(
      `[${apiName}] step 1/2: load Domestic Partnership list (all employees)`
    );
    ...
    const employees = parseDomesticPartnerListXml(listBody);
      ...
      for (let p = 0; p < pageLimit; p++) {
        ...
        const batchResults = await mapPool(page, concurrency, async (emp) => {
          ...
          try {
            ...
            const byWid = parseDomesticPartnerDetailXml(detailBody);
