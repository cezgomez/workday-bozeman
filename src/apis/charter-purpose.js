...
import {
  parseBenefitEventListXml as parseCharterPurposeListXml,
  parseBenefitEventDetailXml as parseCharterPurposeDetailXml,
...
const LIST_REPORT = 'CR_Export_HR_Charter___Purpose_-_Copy';

const DETAIL_REPORT = 'CR_Export_HR_Charter___Purpose';

/**
 * charter-purpose — HR Charter & Purpose attachments
 *
 * Requirements lines 1152–1214:
 *   1. CR_Export_HR_Charter___Purpose_-_Copy → EmployeeID + workdayID + WDWID
 *   2. CR_Export_HR_Charter___Purpose (bulk or Worker!WID) → Attachment + Base64
 *   3. Match WDWID / Attachment WID
 *   4. Charter_And_Purpose/100_{EmployeeId}_CharterAndPurpose_{AttachmentNoSpaces}
 *   5. SFTP /ROI/Workday/
 *
 * Example:
 *   Charter_And_Purpose/100_44492_CharterAndPurpose_Scan05-05-2025-141232.pdf
 */
export async function runCharterPurpose(options = {}) {
  const {
    mock = false,
    pageSize = 5,
    maxPages = null,
  ...
  const apiName = 'charter-purpose';
  const directoryName = 'Charter_And_Purpose';
  const filePrefix = 'CharterAndPurpose';
  ...
  const listUrl =
    process.env.WORKDAY_CHARTER_PURPOSE_LIST_URL || buildRaasUrl(LIST_REPORT);
  const detailUrl =
    process.env.WORKDAY_CHARTER_PURPOSE_DETAIL_URL || buildRaasUrl(DETAIL_REPORT);

  try {
    if (mock) {
      throw new Error(
        'charter-purpose --mock true is not implemented. Use --mock false.'
      );
    }
    ...
    console.log(
      `[${apiName}] step 1/2: load Charter & Purpose list (all employees)`
    );
    ...
    const employees = parseCharterPurposeListXml(listBody);
    ...
    console.log(`[${apiName}] step 2/2: detail RaaS → save + SFTP`);
    console.log(`[${apiName}]   ${detailUrl}`);

    // Prefer bulk detail (curl has no Worker filter); fall back to per-worker.
    let bulkByWid = null;
    try {
      ...
      console.log(
        `[${apiName}] bulk detail response received (${detailBody.length} chars)`
      );
      bulkByWid = parseCharterPurposeDetailXml(detailBody);
      console.log(
        `[${apiName}] bulk detail attachments with Base64: ${bulkByWid.size}`
      );
    } catch (err) {
      console.log(
        ...
        const batchResults = await mapPool(page, concurrency, async (emp) => {
          ...
          try {
            ...
            if (!byWid || byWid.size === 0) {
              ...
              byWid = parseCharterPurposeDetailXml(detailBody);
            }
