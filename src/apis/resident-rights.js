...
import {
  parseBenefitEventListXml as parseResidentRightsListXml,
  parseBenefitEventDetailXml as parseResidentRightsDetailXml,
...
const LIST_REPORT = 'CR_Export_Resident_Rights_-_Copy';

const DETAIL_REPORT = 'CR_Export_Resident_Rights';

/**
 * resident-rights — Resident Rights signed form attachments
 *
 * Requirements lines 1087–1149:
 *   1. CR_Export_Resident_Rights_-_Copy → EmployeeID + workdayID + WDWID
 *   2. CR_Export_Resident_Rights?Worker!WID=… → Attachment + Base64
 *   3. Match WDWID / Attachment WID
 *   4. Resident_Rights/100_{EmployeeId}_ResidentRights_{AttachmentNoSpaces}
 *   5. SFTP /ROI/Workday/
 *
 * Example:
 *   Resident_Rights/100_49129_ResidentRights_ResidentRights.pdf
 */
export async function runResidentRights(options = {}) {
  const {
    mock = false,
    pageSize = 5,
    maxPages = null,
  ...
  const apiName = 'resident-rights';
  const directoryName = 'Resident_Rights';
  const filePrefix = 'ResidentRights';
  ...
  const listUrl =
    process.env.WORKDAY_RESIDENT_RIGHTS_LIST_URL || buildRaasUrl(LIST_REPORT);
  const detailUrl =
    process.env.WORKDAY_RESIDENT_RIGHTS_DETAIL_URL || buildRaasUrl(DETAIL_REPORT);

  try {
    if (mock) {
      throw new Error(
        'resident-rights --mock true is not implemented. Use --mock false.'
      );
    }

    const wd = getWorkdayConfig();

    console.log(`[${apiName}] step 1/2: load Resident Rights list (all employees)`);
    ...
    const employees = parseResidentRightsListXml(listBody);
      ...
      for (let p = 0; p < pageLimit; p++) {
        ...
        const batchResults = await mapPool(page, concurrency, async (emp) => {
          ...
          try {
            ...
            const byWid = parseResidentRightsDetailXml(detailBody);
